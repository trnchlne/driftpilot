/**
 * Backtest: Compare R-fast with and without reversion trailing stop.
 * Tests on 90d SOL data and HYPE data.
 */
...
// Build configs: with trail (current code) vs without trail (reversionSlMultiple only)
// "Without trail" = simulate old behavior by using a very large trail delay (999999s)
const withTrail = { ...rFastCfg, name: 'with-trail', feedId: SOL_FEED_ID };
const noTrail = { ...rFastCfg, name: 'no-trail', feedId: SOL_FEED_ID, trailDelaySeconds: 999999 };

function printResults(label: string, ticks: Tick[]) {
  ...
  console.log(`${'Config'.padEnd(14)} | ${'ROI'.padStart(8)} | ${'Trades'.padStart(6)} | ${'WR%'.padStart(5)} | ${'PF'.padStart(5)} | ${'Sharpe'.padStart(7)} | ${'MaxDD'.padStart(6)} | ${'PnL'.padStart(8)} | Exit Reasons`);
  console.log('─'.repeat(110));

  for (const cfg of [withTrail, noTrail]) {
    ...
    console.log(`${cfg.name.padEnd(14)} | ${roi.padStart(8)} | ${String(r.trades).padStart(6)} | ${r.wr.toFixed(1).padStart(5)} | ${r.pf.toFixed(2).padStart(5)} | ${r.sharpe.toFixed(3).padStart(7)} | ${r.maxDd.toFixed(1).padStart(5)}% | ${pnl.padStart(8)} | ${reasons}`);
  }
}

// SOL 90d
try {
  const sol90 = loadTicks(new URL('../data/sol-history-90d.jsonl', import.meta.url).pathname, SOL_FEED_ID);
  if (sol90.length > 100) printResults('SOL 90d', sol90);
} catch { console.log('No SOL 90d data'); }

// SOL 2y
try {
  const sol2y = loadTicks(new URL('../data/sol-history-2y.jsonl', import.meta.url).pathname, SOL_FEED_ID);
  if (sol2y.length > 100) printResults('SOL 2y', sol2y);
...
try {
  ...
  if (hype.length > 100) printResults('HYPE', hype);
